...
import numpy as np
import random
from visual.visualize import visualize_layout, visualize_layout_generation, create_layout_animation, plot_rl_vs_ga_parameters
...
def main():    
    ...
    for run in range(runs):
        ...
        for OptimizerClass in optimizer_list:
            ...
            if optimizer_name == "RLGeneticAlgorithm":
                plot_rl_vs_ga_parameters(results, "rl_vs_ga_parameters.png")
            ...
            if decoded_evolution:
                create_layout_animation(
                    ...
                    output_filename=f"layout_evolution_{optimizer_name.lower()}_2.gif",
                    duration=1.0,
                )
            else:
    ...
    # --- RL-GA ---
    print(f"\n→ RL-GA (Run {best_rl_idx + 1}): Fitness = {best_rl_fitness:.2f}")
    rl_machines = comparison_results['RLGeneticAlgorithm']['final_machines'][best_rl_idx]

    for machine in rl_machines:
        print(f"[RL-GA] Machine {machine.id}: "
            f"Position=({machine.position.x:.2f}, {machine.position.y:.2f}), "
            f"Rotation={machine.rotation:.1f}°")

    # --- Standard GA ---
    print(f"\n→ Standard GA (Run {best_ga_idx + 1}): Fitness = {best_ga_fitness:.2f}")
    ga_machines = comparison_results['GeneticAlgorithm']['final_machines'][best_ga_idx]

    for machine in ga_machines:
        print(f"[GA] Machine {machine.id}: "
            f"Position=({machine.position.x:.2f}, {machine.position.y:.2f}), "
            f"Rotation={machine.rotation:.1f}°")
