...
class RLGeneticAlgorithmRand(GeneticAlgorithm):
    """
    Same RL-controlled genetic algorithm pipeline as RLGeneticAlgorithm
    (algo.gen_algo_rl_init), except local search is applied to random
    crossover offspring instead of to the elites each generation
    (mirroring how the plain GeneticAlgorithm in algo.gen_algo_init
    ...
    def __init__(
        ...
        # Probability to apply local search on a random crossover offspring each generation
    ...
    def optimize(self, initial_population: Optional[List[np.ndarray]] = None) -> Tuple[List[Machine], float, Dict]:
        print("Starting Reinforcement Learning - Genetic Algorithm (random-crossover local search)")
        ...
        for generation in range(self.generations):
            ...
            if generation > 0 and prev_action is not None:
                if not (np.isfinite(prev_best_fitness) and np.isfinite(best_fitness_curr)):
                    # Whole population still infeasible: the ratio below would be
                    # inf/inf = NaN, which would permanently poison the Q-table.
                    reward = 0.0
                elif prev_best_fitness > best_fitness_curr:
            ...
            # Elitism (rate is also controllable) - no local search applied here
            elite_count = max(1, int(self.elitism_rate * self.population_size))
            elite_indices = np.argsort(fitness_scores)[:elite_count]
            for idx in elite_indices:
                new_population.append(population[idx].copy())

            # Fill the rest via crossover, with local search applied to a
            # random crossover offspring (RL-controlled probability)
            while len(new_population) < self.population_size:
                ...
                # Local search on a random crossover offspring
                if random.random() < self.local_search_prob:
                    c1 = self.local_search(c1, max_iterations=5)
